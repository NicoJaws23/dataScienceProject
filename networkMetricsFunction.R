...
metrics <- function(g, cutoff, clusterType = c("undirected", "global", "globalundirected", "localundirected", "local", 
"average", "localaverage", "localaverageundirected", "barrat", "weighted")){
  ...
  outStrength <- igraph::strength(s1G, mode = "out")
  betweenness <- igraph::betweenness(g, cutoff = -1)
  eigenvectorCentrality <- as.data.frame(eigen_centrality(g))
  clusteringCoefficient <- igraph::transitivity(s1G, type = clusterType)
  
  x <- data.frame(In.Degree = inDegree, Out.Degree = outDegree, In.Strength = inStrength, 
                  Out.Strength = outStrength, Betweenness = betweenness, 
...
met <- metrics(s1G, cutoff = -1, clusterType = "local")
